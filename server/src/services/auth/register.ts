...
export async function verifyToken(
  verificationTokenPayload: VerifyTokenPayload
) {
  try {
    await verificationTokenPayloadSchema.validate(verificationTokenPayload);
    const user = await db
      .table("users")
      .select("verification_token", "token_expiration_date", "email")
      .where({ email: verificationTokenPayload.email });
    if (user.length !== 0) {
      if (user[0].verification_token === verificationTokenPayload.token) {
        if (new Date(Date.now()) < user[0].token_expiration_date) {
          ...
          await db
            .table("users")
            .update({ verification_token: null, token_expiration_date: null })
            .where({ email: user[0].email });
...
export async function register(userPayload: UserPayload) {
  try {
    ...
    const user = await userPayloadSchema.validate({
      firstName,
      lastName,
      email,
      password,
      ...
      if (existingUser.length !== 0 && !existingUser[0].active) {
        await db
          ...
          .update({
            ...
            token_expiration_date: verificationTokenExpirationDate,
          })
          .where({ email: user.email });
      }
      await sendMail(mailParams);
      const output =
        existingUser.length !== 0 && !existingUser[0]?.active
          ? existingUser[0]
          : await db
              .table("users")
              .insert({
