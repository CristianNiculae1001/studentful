...
export async function login(payload: LoginPayload) {
  try {
    ...
    const token = jwt.sign(
      { email: userByEmail[0].email, id: userByEmail[0].id },
...
export async function forgotPassword(email: string) {
  try {
    ...
    const URL = `http://localhost:5174/change-password/${extension}/${userByEmail[0]?.id}`;
    await db
      .table("users")
      .update({ requested_reset_password: true })
      .where({ email, active: true });
    ...
    if (response?.status) {
      return {
        status: 1,
        message:
          "Email has been sent, check your email and follow the next steps.",
      };
    } else {
      return {
        status: 0,
...
export async function changePassword(payload: {
  ...
  try {
    const user = await db
      .table("users")
      .select("id", "email", "password", "requested_reset_password")
      .where({ id: payload.id });
    if (user.length === 0) {
      return {
        status: 0,
    ...
    await db
      .table("users")
      .update({ password: hashedPassword, requested_reset_password: false })
      .where({ id });
    return {
      status: 1,
  ...
  try {
    await db.transaction(async (trx) => {
      ...
      await trx
        ...
        .whereIn(
          "note_id",
          trx.table("note").select("id").where({ user_id: id })
        )
        .del();
