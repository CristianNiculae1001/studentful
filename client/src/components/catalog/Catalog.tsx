import { Box, Button, Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerFooter, DrawerHeader, DrawerOverlay, FormControl, FormLabel, HStack, IconButton, Input, useDisclosure, useToast } from "@chakra-ui/react"
      ...
      >
        ...
        <DrawerContent>
          ...
          <DrawerBody>
            ...
            <Box className="semesterContainer" mt={'1rem'}>
              <Box display={'flex'} justifyContent={'space-between'} gap={'1rem'}>
                ...
                <Box>
                  <IconButton icon={<FiPlus />} aria-label="Add Course" fontSize={16} isRound colorScheme={'green'} onClick={() => {
                    setSem1Data((prev) => {
                      const newData = {
                        sem1: [
                          ...prev.sem1,
              ...
              <Box>
                {
                  sem1Data.sem1.map((element, index) => (
                    ...
                    <FormControl isRequired>
                        ...
                        setSem1Data((prev) => {
                          const updatedElement = prev.sem1.map(el => {
                            if(el.id === element.id) {
                              return {
                                ...el,
                                note: e.target.value.split(',').map(e => +e),
                              }
                            } else {
                              return el;
                            }
                      ...
                      }} value={element.note.join(',')} placeholder="6,7,8,10" />
                    </FormControl>
                        ...
                        setSem1Data((prev) => {
                          const updatedElement = prev.sem1.map(el => {
                            if(el.id === element.id) {
                              return {
                                ...el,
                                credite: e.target.value.length === 0 ? null : +e.target.value,
                              }
                            } else {
                              return el;
                            }
              ...
              <Box>
                {
                  sem2Data.sem2.map((element, index) => (
                    ...
                    <Box mt={'1.5rem'}>
                      <IconButton icon={<IoRemoveOutline />} aria-label="Delete Entry" isRound colorScheme="red" color={'#fff'} fontSize={18} onClick={() => {
                        setSem2Data((prev) => {
                          const updatedData = prev.sem2.filter(e => e.id !== element.id);
          ...
          <DrawerFooter>
            ...
            <Button colorScheme='blue' isDisabled={anLabel.length === 0 || sem1Data.sem1.length === 0 || sem2Data.sem2.length === 0 || sem1Data.sem1.map(e => e.name).some(e => e.length === 0) || sem2Data.sem2.map(e => e.name).some(e => e.length === 0)} onClick={handleAddCatalogData}>Save</Button>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
    </Box>
