...
function Calculator() {
	...
	const borderColorValue = useColorModeValue('gray.100', 'gray.900');
	const bgColorValue = useColorModeValue('gray.100', 'gray.900');

	const mean =
		calculatorData
			.map((e) => e.medie * (e.nrCredite ?? 1))
			.reduce((a, b) => a + b, 0) /
		(calculatorData.map((e) => e.nrCredite ?? 0).reduce((a, b) => a + b, 0) ||
			1);
			...
			<Box className='menuContainer' textAlign={'end'}>
				...
				<CSVLink
					...
					style={{
						...
						borderRadius: '1rem',
						pointerEvents: calculatorData.length === 0 ? 'none' : 'auto',
					}}
					title={
						calculatorData.length === 0
							? 'Adauga cel putin o inregistrare pentru a putea exporta CSV'
							: undefined
					}
					filename='medii.csv'
							...
							<HStack key={index} gap={'1rem'} mb={'1rem'}>
								...
								<FormControl>
									...
									<Input
										...
										defaultValue={element.nrCredite || 0}
										min={0}
									/>
								</FormControl>
