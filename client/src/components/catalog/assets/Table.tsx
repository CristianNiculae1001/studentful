...
function Table({isSuccessful, setIsSuccessful}: {isSuccessful: boolean; setIsSuccessful: React.Dispatch<React.SetStateAction<boolean>>}) {
    ...
    const handleCatalogUpdate = async (event?: CellValueChangedEvent<any, any>) => {
        const updatedData: Record<string, {sem1: {credite: number; id: string; name: string; note: number[]}[]; sem2: {credite: number; id: string; name: string; note: number[]}[];}>[] = [];
        const groupedDataByAn: Record<string, {sem1: {credite: number; id: string; name: string; note: number[]}[]; sem2: {credite: number; id: string; name: string; note: number[]}[];}> = {};
        let iterator = event ? event.api.getRenderedNodes() : gridRef.current && gridRef?.current.api.getRenderedNodes();
        ...
        const result = await updateCatalog(sessionStorage.getItem('auth') ?? '', updatedData);
        if(result?.status) {
            toast({
